7. Math Potato
Rework the previous problem by using a PriorityQueue so that a child is removed only on a composite (not prime) cycle (cycles start from 1). (Accept that "1" is a composite number <- Which was not mentioned previously)
